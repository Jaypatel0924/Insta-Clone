...
export const addNewPost = async (req, res) => {
    try {
        const { caption, location, taggedUsers, mentions } = req.body;
        ...
        try {
            ...
            if (location) {
                try {
                    ...
                    console.log('Invalid location data:', e.message);
                }
            }
            ...
            if (taggedUsers) {
                try {
                    ...
                    // Ensure all are valid MongoDB IDs
                    if (Array.isArray(parsedTaggedUsers) && parsedTaggedUsers.length > 0) {
                        postData.taggedUsers = parsedTaggedUsers.filter(id => 
                            id && typeof id === 'string' && id.length === 24
                        );
                    }
                } catch (e) {
                    console.log('Invalid tagged users:', e.message);
                }
            }
            ...
            if (mentions) {
                try {
                    ...
                    if (Array.isArray(parsedMentions) && parsedMentions.length > 0) {
                        postData.mentions = parsedMentions.filter(id => 
                            id && typeof id === 'string' && id.length === 24
                        );
                    }
                } catch (e) {
                    console.log('Invalid mentions:', e.message);
                }
            }
            ...
            const populatedPost = await Post.findById(post._id)
                ...
                .populate({ path: 'mentions', select: 'username profilePicture' });
            ...
            console.log('Sharp/Cloudinary error:', imageError.message);
            throw imageError;
        }
